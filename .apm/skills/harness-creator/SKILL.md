...
description: "コード・作業の検証ハーネス(機械的に駆動して正しさを判定する自家製の仕組み)を構築する。「ハーネスを作って」「RED/GREENで観測したい」「再発防止のフックを差し込みたい」で使う。unit harness(hook/CI/lint)と system harness(REPF)の2形態。AI出力品質の評価ハーネスは対象外(ai-prompt-improvement-harness等の仕事)。"
---
